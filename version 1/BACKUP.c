...
static struct ctimer renew_key_timer;
...
void encrypt(const uint8_t *key, int key_size, uint8_t *ciphertext, int cipher_size, int packet_number, char* type) {

    uint8_t associated_data[8] = {packet_number};
    uint8_t plaintext[16];

    strncpy((char*)plaintext, type, sizeof(plaintext) - 1);
...
void decrypt(const uint8_t *key, int key_size, const uint8_t *ciphertext, int cipher_size){
                                  ...
                                  uint16_t datalen)
{
    ...
    if (device_number != -1){
        ...
        decrypt(device_list[device_number].current_key, 16, ciphertext, 24);
    }
...
static void send_renew_key(void *ptr)
{
    for (int i = 0; i < 5; i++){
        if (device_list[i].filled){
            udp_packet_t packet;
            ...
            device_list[i].sequence_number +=1;

            encrypt(device_list[i].current_key, sizeof(device_list[i].current_key), ciphertext, 24, device_list[i].sequence_number, "Renew");

            for (int i = 0; i < 24; i++) {
                packet.payload[i] = ciphertext[i];
            }

            packet.sequence_number = device_list[i].sequence_number;

            packet.timestamp = clock_time();

            packet.partial_nonce = iv;

            simple_udp_sendto(&unicast_conn, &packet, sizeof(packet), &device_list[i].link_local_address);
        }
    }

    // Reset the timer for the next send 
    ctimer_reset(&renew_key_timer);
}
...
static void send_first_unicast(int device_number)
{
    ...
    initialize_AES(device_list[device_number].current_key);
    //renew_key(ikm, sizeof(ikm), okm, sizeof(okm), 1, device_number);
...
PROCESS_THREAD(smart_meter_process, ev, data)
{
    ...
    while(1) {
        ...
        ctimer_set(&renew_key_timer, 100 * CLOCK_SECOND, send_renew_key, NULL);
